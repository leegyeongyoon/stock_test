"""Binance USD-M Perpetual 거래소 연결"""
...
class BinancePerpExchange(BaseExchange):
    """Binance USD-M Perpetual 거래소 클라이언트"""
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    "Binance Perp connected",
                    testnet=self.testnet,
                )
                return True

            logger.error("Binance Perp connection failed", status=response.status_code)
            return False

        except Exception as e:
            logger.error("Binance Perp connection error", error=str(e))
            return False
    ...
    async def disconnect(self) -> None:
        ...
        logger.info("Binance Perp disconnected")
    ...
    async def get_ticker(self, symbol: str) -> Optional[TickerData]:
        ...
        price_data = await self._request("GET", "/fapi/v1/ticker/24hr", {"symbol": symbol})
    ...
    async def get_orderbook_depth(
        ...
        """오더북 깊이 조회"""
        data = await self._request("GET", "/fapi/v1/depth", {"symbol": symbol, "limit": limit})
        if not data:
            return None
    ...
    async def get_liquidation_price(self, symbol: str) -> Optional[float]:
        ...
        if positions:
            return positions[0].liquidation_price
        return None
    ...
    async def get_all_usdt_symbols(self) -> list[str]:
        ...
        logger.info(
            "Fetched USDT perpetual symbols",
            count=len(symbols),
        )
        return symbols
    ...
    async def get_all_tickers(self) -> dict[str, dict]:
        """
        전체 심볼 24h 티커 일괄 조회
        GET /fapi/v1/ticker/24hr (weight: 40 for all)
    ...
    async def get_all_funding_rates(self) -> dict[str, float]:
        """
        전체 심볼 펀딩비 일괄 조회
        GET /fapi/v1/premiumIndex (weight: 1 for all)

        Returns:
            심볼별 펀딩비 {symbol: funding_rate}
        """
    ...
    async def get_all_book_tickers(self) -> dict[str, dict]:
        """
        전체 심볼 호가 일괄 조회
        GET /fapi/v1/ticker/bookTicker (weight: 2 for all)
